...
- 仓库根目录：`D:\Users\书玉\Desktop\creating\medical-resume-agent`
...
网页端继续交付“医学经历编译器”：将用户确认的真实经历，转换为四个目标方向下可直接用于简历的候选要点。Skill 端同时支持完整高密度简历：先按事实规划不同内容维度，再生成稳妥、专业和高竞争力三档版本；条目数量随证据充分程度变化，不得用重复或虚构内容凑数。
