...
当前 SQL 职业卡导入药物警戒 / 药物安全、法规医学写作、临床研究协调 / CRA 支持、临床数据管理 / CDM 支持、医疗器械临床 / 应用支持五条已有冻结证据的方向；每条都可回溯至仓库内保留的 JD 摘录，不等于实时岗位库或自动匹配结论。另有 synthetic-only 的解释查询 MVP，统一返回 direct / transferable / partial / gap / unsupported，不产生适配分数。
