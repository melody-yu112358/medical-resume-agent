...
SQL 职业卡可回溯至仓库内保留的 JD 摘录，不等于实时岗位库或自动匹配结论；当前卡片集合及解释支持范围只见上述生成状态页。Synthetic-only 解释查询按[解释契约](docs/CAREER_EXPLANATION_CONTRACT.md)返回 direct / transferable / partial / gap / unsupported 非互斥标签，不产生适配分数。
