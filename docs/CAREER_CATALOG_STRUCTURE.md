...
目录完整性仍有未回答的范围问题：是否只收录医疗行业内转岗、是否包含非医疗行业通用岗位、哪些医学背景/地区/职业阶段属于覆盖范围。没有这些边界和可复核的 JD 样本分母，就没有可靠的“85%”。后续市场补缺应另立研究任务，不能由本次文件盘点推断市场空白。

## 7. 下一步优先级与完成条件

| 优先级 | 工作 | 完成条件 | 本次是否实施 |
| --- | --- | --- | --- |
| P0 | 修正资产/规划文档漂移 | PV/RMW 当前状态一致；不把 75–85% 当已测得覆盖率；申请路径不混入职业计数 | 本次文档修正 |
| P1 | 梳理一个最容易混淆的角色簇：临床科研 / CRA 支持 / 临床运营协调 | 用现有 JD 对照任务、交付物、机构与责任边界；每条关系明确为等价/子范围/相邻/待确认，支持或否定拆分都有依据 | 仅提出；不改 Pack |
| P1 | 对已有三张无规则 Card 排定解释试点 | CRA、PV、RMW 分别列出 claim 规则缺口、人工来源支持与 synthetic 正/部分/负例；先选一个，不并行扩所有方向 | 仅提出 |
| P2 | 整理医学事务与健康科技的宽范围 | 把 MSL、医学信息、数据/产品等已提及概念的关系核清；不先按名称创建一批 Pack | 仅提出 |
| P2 | 扩目录的研究覆盖 | 明确地区、人群、时间窗、来源采样和去重方式；对已有规划方向登记覆盖缺口，再选择新增顺序 | 仅提出；不宣称岗位需求排序 |

优先级按当前结构歧义、已有资产可复用程度制定，不是市场需求、录用概率或用户适配排序。当前两种 Profile 解释方向继续保持原边界。
