...
<p align="center"><strong>中文说明</strong> · <a href="#english">English</a></p>
...
医学简历的难点通常不是经历太少，而是已有经历没有被翻译成目标方向能够判断的能力证明。比如“参与科研”“协助数据分析”并不能说明研究的是什么问题、用了什么方法、承担了哪一环，或留下了什么可以核对的产出。

Skill 的作用不是把经历写得更夸张，而是先把真实经历整理成一张事实卡，再用目标方向关心的语言重新组织重点：

> 真实经历 → 研究对象 / 方法 / 工具 / 实验技术 / 个人角色 / 交付物 → 可迁移能力 → 目标方向的价值表达

所有表达仍以用户确认的事实为边界。缺少的信息会变成问题或“待补充”，不会被补写成责任、数字或成果。
...
默认交付“专业版”表达；同一组已确认事实还可生成稳妥版与高竞争力版供用户比较。

### 核心能力

**经历事实卡。** 从原始描述中识别研究对象、方法、工具、实验技术、个人角色、交付物和待确认信息；每轮最多追问 3 个最关键的问题。

**目标方向翻译。** Skill 覆盖多个医学职业方向，并按同一组已确认事实调整表达重心。仓库内的 canonical machine truth 以 `data/role-packs/*.json` 为准；中文岗位状态索引见 `docs/CAREER_ROLE_PACK_LANDSCAPE.md`。新增 Role Pack 只会通过 canonical source 和自动生成的 Skill projection 扩展，不靠在本说明中手工维护固定数量或完整名单。学术/研究、临床开发与运营、医学事务、数据/健康科技、器械应用等职能族只是使用示例，不承诺每个方向都已成为 runtime target。

**三档改写。** 用同一组已确认事实生成稳妥版、专业版和高竞争力版，帮助比较措辞密度；“参与”不会被无依据升级为“主导”。

**本机交付。** 经确认后生成可打印的 A4 HTML、结构化数据、改写对照与证据摘要；用户可选择提供本地头像图片。
