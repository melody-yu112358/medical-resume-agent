...
## Stage 2: positioning and content plan
...
## Stage 3: three expression tiers
...
## Stage 4: independent factual audit
...
## Stage 5: HTML delivery

```text
Render only the user-accepted resume tier into the bundled ATS HTML template.
Use resume data as the only candidate-information source. Remove unsupported
sections instead of leaving placeholders. Do not expose evidence IDs, audit
notes, risk labels or [待补]. Verify that all placeholders are replaced and keep
the other accepted tiers in resume-data.json for later switching.
```
