...
DIMENSIONS = (("ecosystems", "ecosystem_id", "产业生态"),
              ("lifecycle_stages", "lifecycle_stage_id", "生命周期"),
              ("function_families", "function_family_id", "职能族"))
    ...
    def index():
        ...
        with read_database(database_path) as db:
            ...
            for table, key, title in DIMENSIONS:
                ...
                if request.args.get(table) and request.args[table] not in {x["code"] for x in options[table]}:
                    abort(400, "未知职业地图分类。")
            ...
            filtered = [e for e in entries if all(not request.args.get(t) or request.args[t] in {x["code"] for x in e[t]} for t, _, _ in DIMENSIONS)]
