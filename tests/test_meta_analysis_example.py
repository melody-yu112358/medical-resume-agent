...
class MetaAnalysisExampleTest(unittest.TestCase):
    ...
    def test_extracted_facts_are_consistent(self):
        """提取的事实应该与用户输入一致"""
        # 只能提取用户明确提到的内容
        self.assertIn("meta_analysis", self.example["extracted_facts"]["methods"])
        self.assertIn("retrieve_literature", self.example["extracted_facts"]["actions"])
        self.assertIn("screen_studies", self.example["extracted_facts"]["actions"])

        # 不能推断未提及的内容
        self.assertNotIn("prisma", self.example["extracted_facts"]["methods"])
        self.assertNotIn("database_count", self.example["extracted_facts"]["scope"])
    ...
    def test_four_roles_produce_different_expressions(self):
        ...
        # 检查每个岗位的关键词
        self.assertTrue(any("循证研究方法" in w for w in wordings))  # doctoral
        self.assertTrue(any("临床研究证据" in w for w in wordings))  # clinical_research
        self.assertTrue(any("医学文献" in w for w in wordings))      # medical_affairs
        self.assertTrue(any("医疗研究数据" in w for w in wordings))   # health_ai_data
    ...
    def test_no_forbidden_outputs(self):
        ...
        for forbidden in self.example["forbidden_outputs"]:
            ...
            if "PRISMA" in forbidden:
                self.assertNotIn("PRISMA", all_wordings)
            if "主导" in forbidden:
    ...
    def test_unknowns_are_omitted(self):
        ...
        # 检查数字相关的未知项没有被猜测
        self.assertNotIn("篇", all_wordings)  # study_count
        self.assertNotIn("数据库", all_wordings)  # database_count
    ...
    def test_evidence_traceability(self):
        ...
        for role_pack, claims in self.example["bullet_claims"].items():
            for claim in claims:
                with self.subTest(role_pack=role_pack, claim_id=claim["claim_id"]):
                    self.assertEqual(claim["evidence_ids"], ["ev_001"])
                    self.assertEqual(claim["experience_id"], "exp_001")
